from env_eval_comboStag import Env_eval_comboStag
import functions
import os
import time
import numpy as np
...
for fun in range(1, 29):
    ...
    environment = Env_eval_comboStag(fun, dim=dim, minimum=f_deltas[fun - 1])  # fDeltas[func_num - 1])
    ...
    np.savetxt(f"comboStag_results(30000_3xFEs).csv", results, delimiter=", ", fmt='% s')
