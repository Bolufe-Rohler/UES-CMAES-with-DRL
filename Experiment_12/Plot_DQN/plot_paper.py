...
path = "../D30_all_Combo_stag/"
filename = "D30_all_Combo_stag"
filename_loss = path + "DQN_ComboStag_loss.csv"
filename_re = path +"DQN_ComboStag_returns.csv"
...
ax[0].set_title(f"Reward                          "
                     f"Entire benchmark (reward with stagnation)                     Loss", x=1, y=1, pad=8)

ax[0].plot(rewards, color='b')
ax[1].plot(loss, color='r')

plt.savefig(path + filename + f"_paper.png")
plt.close()
