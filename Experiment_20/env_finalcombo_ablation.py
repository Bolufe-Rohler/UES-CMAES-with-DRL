...
class Env_FinalCombo_Ablation(py_environment.PyEnvironment):
    ...
    ✅ Extended for Exp20:
      - Adds "normalized" reward mode
      - Adds CRN hooks for paired evaluation:
          set_eval_context(fun_num, episode_seed)
    ...
    Notes:
      ...
      - Each reset chooses a random function index in [1..28] (unless forced for eval)
    """
    ...
    def __init__(
        ...
        reward_mode: str = "standard",  # "standard" | "normalized" | "stagnation"
        tau: int = 3,
        ...
        if reward_mode not in ("standard", "normalized", "stagnation"):
            raise ValueError("reward_mode must be 'standard', 'normalized', or 'stagnation'")
        ...
        self._minimums = minimums  # needed for "standard"/"normalized" first-step scaling
        self._minimum = None
        ...
        # Optional penalty for no-improvement steps
        ...
        # ✅ CRN / paired evaluation hooks
        self._forced_fun_num = None
    ...
    # ✅ Used by Exp20 evaluation script
    def set_eval_context(self, fun_num: int, episode_seed: int):
        """
        Force the function index and RNG seed used inside reset()/step().

        Intended use (paired/CRN eval):
          env.set_eval_context(fun_num=f, episode_seed=seed_base + rep)
          ts0 = env.reset()
          ... run one episode ...
        """
        self._forced_fun_num = int(fun_num)
    ...
    def _reset(self):
        ...
        # ✅ CRN: force RNGs if requested
        if self._forced_episode_seed is not None:
            random.seed(self._forced_episode_seed)
            np.random.seed(self._forced_episode_seed)

        # ✅ CRN: force function if requested, else randomize as usual
        if self._forced_fun_num is not None:
            self._fun_num = self._forced_fun_num
        elif self._randomize_function_each_episode:
            self._fun_num = random.randint(1, 28)

        # For standard/normalized reward, we need a function minimum (your fDeltas list)
        if self._reward_mode in ("standard", "normalized"):
            if self._minimums is None:
                raise ValueError("minimums list is required when reward_mode in {'standard','normalized'}")
        ...
        self._state, self._observation, best_f = ues_cmaes_X(
            ...
            include_single_run_features=self._include_single_run_features,
        )
    ...
    def _compute_reward(self, best_f: float) -> float:
        ...
        Normalized (Exp20):
          - first step: (minimum - best_f) / max(|minimum|, 1)
          - later: (best_prev - best_f) / max(|best_prev - minimum|, eps), if improved, else 0
        ...
        if self._actions_count == 1:
            ...
            if self._reward_mode == "normalized":
                denom = max(abs(float(self._minimum)), 1.0)
                r = float((self._minimum - best_f) / denom)
                self._best_fitness = best_f
                return r

            # stagnation mode: match Exp12 style
        ...
        if best_f < self._best_fitness:
            ...
            if self._reward_mode == "normalized":
                denom = max(abs(float(self._best_fitness - self._minimum)), eps)
                improvement = improvement / denom
