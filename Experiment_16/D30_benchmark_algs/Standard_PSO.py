...
class Particle:
    ...
    def __init__(self, fun_num, RangeF=100):
        ...
        self.velocity = np.array([
                                     0] * dimension)  # zero for initial velocity -- A. Engelbrecht, "Particle Swarm
...
class ParticleSwarmOptimizer:
    def __init__(self, fun_num, maxevals, swarmsize):
        self.swarmsize = swarmsize
        self.maxevals = maxevals
        self.iteration = math.floor(maxevals / swarmsize)
        self.swarm = []
    ...
    def Lbest(self, pindex):
        adj_p = [self.swarm[(pindex - 1 + self.swarmsize) % self.swarmsize], self.swarm[pindex],
                 self.swarm[(pindex + 1) % self.swarmsize]]  # index wise adjacent particles
