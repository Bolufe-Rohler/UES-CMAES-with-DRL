...
class Env_Resdisnew60_cnvRan5(py_environment.PyEnvironment):
    def __init__(self, func_num, dim, minimum):
        super().__init__()  # supposedly not needed
        self._action_spec = array_spec.BoundedArraySpec(
            shape=(), dtype=np.int32, minimum=0, maximum=4, name='action')
        self._observation_spec = array_spec.BoundedArraySpec(
            shape=(60,), dtype=np.float64, name='observation')
        self._observ_size = 20
    ...
    def _reset(self):
        ...
        self._state, self._observation, best_f = ues_cmaes_rl(self.obj_function, dim=self._dim, max_eval=self._max_evals,
                                                              bound=100, observ_size=self._observ_size,
                                                              start_point=None, action=None)
        self._best_fitness = None
    ...
    def _step(self, action):
        ...
        #if adjusted_action == 100:
        #    adjusted_action = 99
        self._state, self._observation, best_f = ues_cmaes_rl(self.obj_function, dim=self._dim, max_eval=self._max_evals,
                                                              bound=100, observ_size=self._observ_size,
                                                              start_point=self._state, action=action)
