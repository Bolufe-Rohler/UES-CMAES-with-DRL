...
from env_rdnplus_combo_stagnation import Env_Rdnplus_combo_stagnation
import numpy
import reverb
import os
import time
...
TAU   = int(os.environ.get("TAU"))

# Creating environments
# func_num decided by environment variable (or None for random)
FUNC_NUM = os.environ.get("FUNC_NUM")
if FUNC_NUM is not None:
    func_num = int(FUNC_NUM)
else:
    func_num = None

experiment = f"DQN_ComboStag_gamma{GAMMA}_tau{TAU}"

# Add function number suffix if applicable
if func_num is not None:
    experiment = f"{experiment}_F{func_num}"
...
num_iterations = int(os.environ.get("NUM_ITER"))
...
environment = Env_Rdnplus_combo_stagnation(
    func_num=func_num,
    dim=dim,
    tau=TAU,
    gamma=GAMMA
