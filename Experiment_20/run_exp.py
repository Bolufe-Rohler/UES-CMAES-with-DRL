...
def compute_avg_return(environment, policy, num_episodes=10):
    ...
    for _ in range(num_episodes):
        ...
        # NOTE: relies on internal attribute (same pattern as your previous code)
...
def parse_args():
    ...
    # Training config
    p.add_argument("--num_iterations", type=int, default=200000)  # TARGET TOTAL steps
    ...
    # Exp20a: reduce filesystem overhead from frequent SavedModel exports
    p.add_argument(
        "--save_policy_interval",
        type=int,
        default=10,
        help="Save policy (SavedModel) every K evaluations. 0 disables periodic saves.",
    )
    p.add_argument(
        "--save_policy_at_end",
        type=int,
        default=1,
        help="If 1, save policy once at the end of training.",
    )
...
def _assert_finite_batch(experience):
    ...
    for t in flat:
        if t is None:
            continue
        if hasattr(t, "dtype") and t.dtype.is_floating:
...
def main():
    ...
    # -------------------------------------------------------------------------
    # IMPORTANT: separate Python env instances for training & eval
    # -------------------------------------------------------------------------
    ...
    agent = dqn_agent.DqnAgent(
        ...
        gradient_clipping=1.0,  # ✅ clipping enabled
    ...
    restored_step = int(agent.train_step_counter.numpy())
    if restored_step > 0:
        print(f"[Resume] Restored training from step={restored_step}. Target total steps={args.num_iterations}.")
    else:
        print(f"[Start] No checkpoint found. Starting fresh. Target total steps={args.num_iterations}.")

    # Initial random collection (only when starting fresh)
    if restored_step == 0:
        ...
        print("[Resume] Skipping initial random collection.")
    ...
    # Train until we reach the desired TOTAL number of training steps (supports resume)
    step = int(agent.train_step_counter.numpy())
    if step >= args.num_iterations:
        print(f"[Info] Already at step={step} >= target={args.num_iterations}. Skipping training loop.")
    else:
        remaining = args.num_iterations - step
        print(f"[Info] Training from step={step} for {remaining} more steps (target={args.num_iterations}).")
        ...
        while step < args.num_iterations:
            ...
            if step % args.eval_interval == 0:
                ...
                # checkpoint often (cheap)
                train_checkpointer.save(global_step)

                # Save policy (SavedModel) less frequently (expensive)
                if args.save_policy_interval > 0:
                    eval_index = step // args.eval_interval
                    if eval_index % args.save_policy_interval == 0:
                        tf_policy_saver.save(policy_dir)

        # Always save once at the end if requested
        if args.save_policy_at_end:
            tf_policy_saver.save(policy_dir)
