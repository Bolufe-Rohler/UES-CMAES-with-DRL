...
path = "../D30_all_Combo_norm/"
filename = "D30_all_Combo_norm"
filename_loss = path + "DQN_ComboNorm_loss.csv"
filename_re = path +"DQN_ComboNorm_returns.csv"


fig_all, ax = plt.subplots(1, 2, figsize=(10, 5))
...
ax[0].set_xticks(ticks=[0, 10, 20, 30, 40, 50])
...
ax[0].set_title(f"Reward                          "
                     f"Entire benchmark (normalized reward)                     Loss", x=1, y=1, pad=8)

ax[0].plot(rewards)
ax[1].plot(loss)

plt.savefig(path + filename + f"_fig_norm_all.png")
plt.close()
