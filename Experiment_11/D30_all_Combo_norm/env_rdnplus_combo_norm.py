...
class Env_Rdnplus_combo_norm(py_environment.PyEnvironment):
    def __init__(self, func_num, dim, minimum, median_errors):
        ...
        self._minimums = minimum
        self._median_errors = median_errors
    ...
    def _reset(self):
        ...
        self._median_error = self._median_errors[self._fun_num - 1]
    ...
    def _step(self, action):
        ...
        if self._best_fitness is None:
            reward = (self._minimum - best_f)/self._median_error
            self._best_fitness = best_f
        else:
            reward = max(self._best_fitness - best_f, 0)/self._median_error  # no penalty in reward
            self._best_fitness = min(self._best_fitness, best_f)

        if math.isinf(reward) or math.isnan(reward):  #v...just in case
            reward = 0
        if self._episode_ended:
